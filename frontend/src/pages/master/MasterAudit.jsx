...
export default function MasterAudit() {
  ...
  const audit = useApi(() => masterApi.audit({ module, page, page_size: 25 }), [module, page])
  ...
  return (
    <>
      <PageHeader title="Platform audit log"
        subtitle="Every state change across every tenant, written by the API rather than the browser." />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4">
        <StatCard label="Entries" value={num(pagination?.total ?? 0)} icon={ScrollText} tone="brand" />
      ...
      <InlineAlert tone="info" className="mb-4">
        Written inside the same transaction as the change it describes, so a rolled-back
        operation leaves no misleading entry behind.
      </InlineAlert>
